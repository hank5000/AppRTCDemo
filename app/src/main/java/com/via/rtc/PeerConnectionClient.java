...
import com.via.rtc.DataChannelObserver.AudioDataChannelObserver;
import com.via.rtc.util.GlobalSetting;
import com.via.rtc.util.LiveViewInfo;
import com.via.rtc.util.LooperExecutor;
import com.via.rtc.thread.SendingLiveViewAudioThread;
import com.via.rtc.thread.SendingLiveViewThread;
import com.via.rtc.thread.SendingThread;
import com.via.rtc.DataChannelObserver.VideoDataChannelObserver;
...
import com.via.rtc.DataChannelObserver.CommunicationChannelObserver;
...
public class PeerConnectionClient {
  ...
  private static final String TAG = "VIA-RTC PCRTCClient";
  ...
  private Runnable mTask = new Runnable() {
    ...
    public void run() {
      ...
      if(preStatus.equalsIgnoreCase("OPEN")) {
        if(communicationChannelObserver.isIdle) {
      ...
      preStatus = communicationChannelObserver.status;
      handler.postDelayed(mTask,1000*60*1);
  ...
  private static final int MAX_CHANNEL_NUMBER = 1;
  ...
  public ServerService ssCallback = null;
  ...
  public void setServerServiceInstance(ServerService ss) {
    this.ssCallback = ss;
  }
...
//    }
  ...
  public void stopChannelSending(int index) {
    ...
    if(sendingLiveViewThreads[index]!=null) {
      ...
      try {
        sendingLiveViewThreads[index].join();
      } catch (InterruptedException e) {
    ...
    if(sendingLiveViewAudioThreads[index]!=null) {
      ...
      try {
        sendingLiveViewAudioThreads[index].join();
      } catch (InterruptedException e) {
  ...
  public final int DATA_CHANNEL_DIVIDE_SIZE_BYTE = 1024;//GlobalSetting.getDataChannelDividedSize();
  //
  ...
  public void kickOutUser() {
      if(this.isCreateSide()) {
        Log.d(TAG,"dodisconnect");
        ssCallback.doDisconnect();
        handler.removeCallbacks(mTask);
      }
  }
  ...
  public void sendRequestAuthentication() {
    ...
    final DataChannel outChannel = communicationChannel;
    ByteBuffer buffer = ByteBuffer.wrap(data.getBytes());
    Log.d(TAG,"sendRequestAuthentication");
    outChannel.send(new DataChannel.Buffer(buffer,false));
  ...
  public void sendAuthencationInformation(String user,String pass) {
    ...
    final DataChannel outChannel = communicationChannel;
    ByteBuffer buffer = ByteBuffer.wrap(data.getBytes());
    Log.d(TAG,"send authencation information");
    outChannel.send(new DataChannel.Buffer(buffer, false));
  }
    ...
    try {
      ...
      for(;;) {
        ...
        if (outChannel != null) {
          ...
          Log.d(TAG, "Send File : " + ((float) sentSize * 100 / (float) fileSize) + "%");
        }
  ...
  public void sendVideoInfo(final int channelIndex,String key, String value) {
    ...
    Log.d(TAG,"Send Video Info "+data);
    outChannel.send(new DataChannel.Buffer(buffer, false));
  ...
  public void sendAudioInfo(final int channelIndex,String key) {
    ...
    ByteBuffer buffer = ByteBuffer.wrap(data.getBytes());
    Log.d(TAG, "Send Audio Info " + data);
    outChannel.send(new DataChannel.Buffer(buffer, false));
  }
  ...
  private void createPeerConnectionInternal() {
    ...
    if(bEnableDataChannel) {
      ...
      //handler.postDelayed(mTask,1000*60);
  ...
  public class EmptyDCObserver implements DataChannel.Observer {
    ...
    public void onBufferedAmountChange(long var1) {
      Log.d(TAG,"DataChannel.Observer onBufferedAmountChange : "+var1);
    }
    @Override
