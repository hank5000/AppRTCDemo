...
public class SendingLiveViewThread extends Thread {
    ...
    public void run() {
        ...
        while (!Thread.interrupted() && bStart ) {
            int naluSize = 0;
            GlobalSetting.member.LOGD("start ");
            try {
                GlobalSetting.member.LOGD("read ");

                //naluSize = in.read(transferBuffer,0,in.available());
                naluSize = in.read(transferBuffer);
                GlobalSetting.member.LOGD("read out : nalusize : "+naluSize);
                ...
                if(true) {
                    GlobalSetting.member.LOGD("is not connect lo.");

                    try {
                        sleep(3500);
                        int Count = 0;
                        while(Count<=10) {
                            GlobalSetting.member.LOGD( "Try to Connect again "+Count);
                            try {
                                localSocket = null;
            ...
            if(naluSize>0) {
                ...
                for (; ; ) {
                    ...
                    GlobalSetting.member.LOGD("Send video size :" + dividedSize);
        ...
        GlobalSetting.member.LOGD("out side");
        while(true) {
            try {
                localSocket.close();
