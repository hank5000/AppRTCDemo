...
public class SendingThread extends Thread {
    final static String TAG = "VIA-RTC SendingThread";
    ...
    public void run() {
        ...
        while (!Thread.interrupted() && bStart) {
            ...
            //Log.d(TAG, "readSampleData : " + sampleSize);
            if (naluSize > 0) {
                NALUCount++;
                //Log.d(TAG, "Send NALU length : " + sampleSize);
                ...
                if((endTime-startTime)<33 && (endTime-startTime)>=0) {
                    try {
                        ...
                        //Log.d(TAG,"Sleep : "+(33-(endTime-startTime))+"ms");
                    } catch (Exception e) {
                ...
                Log.d(TAG, "No Frame lo.");

                if(bAutoLoop) {
                    Log.d(TAG, "Enable Auto Loop");
                    mediaExtractor.seekTo(0, MediaExtractor.SEEK_TO_CLOSEST_SYNC);
                } else {
                    Log.d(TAG, "Auto Loop is disable");
