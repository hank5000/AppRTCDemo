...
import com.via.rtc.thread.VideoThread;
...
public class VideoDataChannelObserver implements DataChannel.Observer {
    final static String TAG = "VIA-RTC VideoDCObserver";
    ...
    public void onStateChange() {
        ...
        if(dataChannel.state().toString().equalsIgnoreCase("CLOSING")) {
            GlobalSetting.member.LOGD(dataChannel.label()+":"+dataChannel.state().toString());
        ...
        if(inbuf.binary)
        {
            ...
            if(bStart) {
                try {
                    ...
                    GlobalSetting.member.LOGD("wrtie to WritableByteChannel");

                } catch (Exception e) {
                    Log.d(TAG, "os write fail:"+e);
                }
            } else {
                Log.e(TAG,"bStart is not enable, but received binary type buffer ?");
            }
        }
        else
        {
                ...
                switch(messageType)
                {
                    ...
                    case STOP:
                    {
                        if (bStart) {
                            ...
                            Log.d(TAG, "It is already stop lo.");
                        }
                        break;
                    }
                }
